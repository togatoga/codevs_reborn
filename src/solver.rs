...
impl Solver {
    ...
    pub fn think(&mut self, current_turn: usize) -> SearchResult {
        ...
        let mut search_state_heap: Vec<Vec<MinMaxHeap<SearchState>>> = (0..beam_depth + 1)
                    ...
                    for (pack, rotate_count) in self.packs[search_turn].iter() {
                        for point in 0..9 {
                            ...
                            if !next_search_state.is_command() {
                                ...
                                next_search_state
                                    .set_command(Command::Drop((point, *rotate_count)));
                            }
                            ...
                            for t in 0..2 {
                                ...
                                let next_search_score = self.evaluate_cache.evaluate_search_score(
                                    &mut self.simulator,
                                    &next_search_state,
                                    t,
                                ) + rnd.randf();
                                ...
                                if chain_count <= 10 {
                                    ...
                                    debug_assert!(
                                        search_state_heap[depth + 1][t].len() <= beam_width
                                    );
                                }
                                ...
                                if target_search_result_score
                                    > best_search_result.search_result_score
                                {
                                    best_search_result.search_result_score =
                                        target_search_result_score;
                                    ...
                                    best_search_result.command =
                                        next_search_state.command().unwrap();
